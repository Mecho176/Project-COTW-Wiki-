...
import { MessageSquare, PenTool, MoreVertical } from 'lucide-react';
...
export default function FurTypesPage() {
  ...
  return (
    <div className="w-full max-w-6xl mx-auto p-8 text-blue-100 bg-[#0B111C] min-h-screen font-sans">
      
      {/* --- HEADER --- */}
      <div className="flex justify-between items-start border-b border-slate-700 pb-4 mb-8">
        ...
        <div className="flex items-center gap-4 text-xs font-bold text-blue-300">
           ...
           <div className="flex items-center gap-1 cursor-pointer hover:text-white">
              <PenTool size={12} />
              <span className="uppercase tracking-wide">Sign In to Edit</span>
           </div>
      ...
      {/* --- INTRO --- */}
      <div className="text-slate-300 space-y-4 mb-8 text-[15px]">
        <p>Example[1]</p>
        <p>
      ...
      {/* --- TABLE --- */}
      <div className="border border-slate-700 rounded-sm overflow-hidden bg-[#0E1624]">
        <table className="w-full text-left border-collapse text-sm">
          <thead>
            <tr className="bg-[#131B29] text-white font-bold border-b border-slate-700 text-sm">
              <th className="p-4 w-40 border-r border-slate-700">Fur Type</th>
              <th className="p-4 w-32 border-r border-slate-700">Number of Animals with the fur type/variation</th>
              <th className="p-4 w-1/2 border-r border-slate-700">Animals</th>
          ...
          <tbody>
            {furData.map((row, index) => (
              <tr 
                key={index} 
                className="border-b border-slate-700/50 hover:bg-slate-800/30 transition-colors align-top"
              >
                {/* Fur Type */}
                <td className="p-4 border-r border-slate-700/50 text-white font-medium">
                  {row.type}
                </td>

                {/* Count */}
                <td className="p-4 border-r border-slate-700/50 text-white text-center font-medium">
                  {row.count}
                </td>

                {/* Animals List */}
                <td className="p-4 border-r border-slate-700/50 text-white">
                  <div className="mb-2 text-slate-300">List :</div>
                  <div className="pl-4">
