...
wikiData.forEach(wikiWeapon => {
    // Normalize string spaces for comparison if needed
    const localWeapon = rifles.find(w => w.name.toLowerCase() === wikiWeapon.name.toLowerCase());
    if (!localWeapon) {
        console.log(`[WARNING] Weapon from Wiki not found locally: ${wikiWeapon.name}`);
        return;
    }

    const fieldsToCheck = ['accuracy', 'recoil', 'reload', 'hipshot', 'magazine'];
    fieldsToCheck.forEach(field => {
        if (localWeapon[field] != wikiWeapon[field]) {
             discrepancies.push({
                weapon: wikiWeapon.name,
                field: field,
                local: localWeapon[field],
                wiki: wikiWeapon[field]
             });
        }
    });
});
...
if (discrepancies.length > 0) {
    ...
    discrepancies.forEach(d => {
        console.log(`${d.weapon} - ${d.field}: Local=${d.local}, Wiki=${d.wiki}`);
    });
} else {
    console.log("No discrepancies found in common fields.");
}
