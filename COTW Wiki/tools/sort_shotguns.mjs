...
const shotguns = weaponsArray.filter(w => w.type === 'Shotgun').slice().sort((a, b) => a.name.localeCompare(b.name));
const nonShotguns = weaponsArray.filter(w => w.type !== 'Shotgun');
const newArray = shotguns.concat(nonShotguns);
...
console.log('Sorted shotguns and updated', weaponsPath);
